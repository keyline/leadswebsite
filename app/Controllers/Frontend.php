...
class Frontend extends BaseController
{
    ...
    public function product($category)
    {
        ...
        foreach ($data['product'] as &$product) {
            $warrenty_ids = (json_decode($product->warrenty_section));        
            $product->others_images = $this->common_model->find_data('product_others_image', 'array', ['published!=' => 3, 'product_id' => $product->id]);            
            $warrenty_section = [];
            foreach($warrenty_ids as $key => $warrenty){                                
                $warrenty_section[] = $this->common_model->find_data('warrenty_section', 'row', ['published!=' => 3, 'id' => $warrenty]);
        }
        $product->warrenty_section = $warrenty_section;
        }
        //  pr($data['product']);
...
<?php }
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            if (!$this->validate($rules)) {
                ...
                $maildata = [
                    ...
                    'qualification' => $postData['qualification'],
                    'experience' => $postData['experience'],
                    'job_id' => $this->common_model->find_data('sms_career','row','sms_career.id = '.$postData['job'],'name'),
                    'cv_file' => $applicantCv,
                ];                
                //  pr($data);
                ...
                if ($insert_id) {
                    ...
                    $body = view('Views/front/mail_template/jobapply-templete', $maildata);
    ...
    public function product_registration()
    {
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            $rule = [
                ...
                'purchase_invoice' => [
                    'rules' => 'max_size[purchase_invoice,10240]|mime_in[purchase_invoice,application/pdf,image/jpg,image/jpeg,image/png]',
                    'label' => 'Purchase Invoice'
                ],
                'barcode_photo' => [
                    'rules' => 'max_size[barcode_photo,10240]|mime_in[barcode_photo,image/jpg,image/jpeg,image/png]',
                    'label' => 'Barcode Photo'
                ]
    ...
    public function demo()
    {
        ...
        // $data['productcat']         = $this->common_model->find_data('product_category', 'array', ['published' => 1]);
        $order_by[0]                = array('field' => 'sort', 'type' => 'asc');
        $data['productcat']   = $this->common_model->find_data('product_category', 'array', ['published' => 1], '', '', '', $order_by);
        if ($this->request->getMethod() === 'post') {
            ...
            if ($this->verifyRecaptcha($_POST['recaptcha_token'])) {
                ...
                $formData = [
                    ...
                    'address'       => $postData['address'],
                    'state'           => $postData['state'],
                    'categorySelect'    => $postData['product_interest'],
                    'productSelect' => $postData['product_list'],
                    'date'          => $postData['date']
                ];
                //  pr($formData);
                ...
                $formData['site_setting'] = $this->common_model->find_data('sms_site_settings', 'row', ['published' => 1]);
                $formData['product_cat'] = $this->common_model->find_data('product_category', 'row', ['published' => 1, 'id' => $postData['product_interest']]);
                // pr($data['product_cat']);
                $formData['product_name'] = $this->common_model->find_data('product', 'row', ['published' => 1, 'id' => $postData['product_list']]);
    ...
    public function get_products_by_category() {
        ...
        if ($category_id) {
            $products = $this->common_model->find_data('product', 'array' ,['published' => 1, 'product_category' => $category_id]);
            // pr($products);
            echo json_encode($products);
        } else {
            echo json_encode([]); // Return empty array if no category selected
        }
    }
    ...
    private  function sendToAdmin($subject = '', $body = '', $sendTo = 'admin', $toName = 'Admin')
    {
        ...
        // $send_email = $sendTo == 'admin' ? 'subhomoy@keylines.net' : 'system@keylines.net';
