...
class manage_key_feature extends BaseController {
	...
	public function __construct()
    {
        ...
        $this->data = array(
            ...
            'session'       => $session,
            'module'        => 'Key Feature',
            'controller'    => 'manage_key_feature',
            'table_name'    => 'key_feature',
            'primary_key'   => 'id'
        );
    }
    ...
    public function add()
    {
        ...
        $page_name                  = 'key_feature/add-edit';        
        $data['row']                = [];
        $data['productCats']           = $this->data['model']->find_data('product_category', 'array', ['published!=' => 3]);
        if($this->request->getMethod() == 'post') {
            //  pr($this->request->getPost());
            $slug = strtolower($this->data['model']->clean($this->request->getPost('key_feature_title')));
            /* key_feature icon upload */
            $file = $this->request->getFile('key_feature_icon'); 
            $originalName = $file->getClientName();
            $fieldName = 'key_feature_icon'; 
            if($originalName!='') {
                $upload_array = $this->common_model->upload_single_file($fieldName,$originalName,'key_feature','image');
            ...
            if($originalName!=''){
                $postData   = array(
                                ...
                                'key_feature_title'         => $this->request->getPost('key_feature_title'),
                                'slug'                      => $slug,                                
                                'key_feature_description'   => $this->request->getPost('key_feature_description'),
                                'key_feature_icon'          => $image                                
                                );
    ...
    public function edit($id)
    {
        ...
        if($this->request->getMethod() == 'post') {            
            ...
            if($originalName!='') {
                ...
                $image = $data['row']->key_feature_icon;
            }
            /* key_feature icon */
            $postData = array(
                    'product_category'          => $this->request->getPost('product_category'),
    ...
    public function image_list($id) 
    {
        ...
        $page_name                  = 'key_feature/image-list';        
    ...
    public function image_add($id)
    {
        ...
        $page_name                  = 'key_feature/add-edit-image';
        $data['row']                = [];
        $data['project_id']           = $id;
        $data['project']              = $this->data['model']->find_data($this->data['table_name'], 'row', ['id' => $id]);       
