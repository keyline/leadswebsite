...
class Manage_investor extends BaseController
{
    ...
    public function add()
    {
        ...
        $page_name = 'investor/add-edit-image';
        $data['row'] = [];
        $data['investor'] = $this->data['model']->find_data('investor', 'array', ['published!=' => 3]);
        if ($this->request->getMethod() == 'post') {
            ...
            $fieldName = 'pdf';
            if ($originalName != '') {
                $upload_array = $this->common_model->upload_single_file($fieldName, $originalName, 'investor', 'pdf');
