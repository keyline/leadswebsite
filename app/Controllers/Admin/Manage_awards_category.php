...
use DB;
class Manage_awards_category extends BaseController {
	...
	public function __construct()
    {
        ...
        $this->data = array(
            'model'         => $model,
            'session'       => $session,
            'module'        => 'Awards Category',
            'controller'    => 'manage_awards_category',
            'table_name'    => 'sms_awards_category',
            'primary_key'   => 'id'
        );
    }
    public function index() 
    {
        ...
        $page_name                  = 'awards-category/list';        
    ...
    public function add()
    {
        ...
        $page_name                  = 'awards-category/add-edit';        
        $data['row'] = [];
        if($this->request->getMethod() == 'post') { 
            /* category icon upload */
            $file = $this->request->getFile('category_icon');
            $originalName = $file->getClientName();
            $fieldName = 'category_icon';
            if($originalName!='') {
                $upload_array = $this->common_model->upload_single_file($fieldName,$originalName,'awards_category','image');
                if($upload_array['status']) {
                    $category_icon = $upload_array['newFilename'];
                } else {
            ...
            $file = $this->request->getFile('category_images');
            $originalName = $file->getClientName();
            $fieldName = 'category_images';
            if($originalName!='') {
                $upload_array = $this->common_model->upload_single_file($fieldName,$originalName,'awards_category','image');
                if($upload_array['status']) {
                    $category_images = $upload_array['newFilename'];
                } else {
                    $this->session->setFlashdata('msg1', $upload_array['message']);
                    return redirect()->to(current_url());
                }
    ...
    public function edit($id)
    {
        ...
        if($this->request->getMethod() == 'post') {  
            ...
            if($originalName!='') {
                ...
                if($data['row']->category_icon!='') {
                    unlink('uploads/awards_category/'.$data['row']->category_icon);  
                }                
                ...
                $category_icon = $data['row']->category_icon;
            }
            /* category icon */
            /* category images upload */
            $file = $this->request->getFile('category_images');
