...
class manage_warrenty_section extends BaseController {
	...
	public function __construct()
    {
        ...
        $this->data = array(
            'model'         => $model,
            'session'       => $session,
            'module'        => 'Warrenty Section',
            'controller'    => 'manage_warrenty_section',
            'table_name'    => 'warrenty_section',
            'primary_key'   => 'id'
        );
    }
    public function index() 
    {
        ...
        $page_name                  = 'warrenty_section/list';        
    ...
    public function add()
    {
        ...
        $page_name                  = 'warrenty_section/add-edit';        
        $data['row']                = [];
        $data['productCats']           = $this->data['model']->find_data('product_category', 'array', ['published!=' => 3]);
        if($this->request->getMethod() == 'post') {
            //  pr($this->request->getPost());
            $slug = strtolower($this->data['model']->clean($this->request->getPost('warrenty_section_title')));
            /* warrenty_section icon upload */
            $file = $this->request->getFile('warrenty_section_icon'); 
            $originalName = $file->getClientName();
            $fieldName = 'warrenty_section_icon'; 
            if($originalName!='') {
                $upload_array = $this->common_model->upload_single_file($fieldName,$originalName,'warrenty_section','image');
            ...
            if($originalName!=''){
                $postData   = array(
                                ...
                                'warrenty_section_title'         => $this->request->getPost('warrenty_section_title'),
                                'slug'                      => $slug,                                
                                'warrenty_section_description'   => $this->request->getPost('warrenty_section_description'),
                                'warrenty_section_icon'          => $image                                
                                );
    ...
    public function edit($id)
    {
        ...
        if($this->request->getMethod() == 'post') {            
            ...
            if($originalName!='') {
                ...
                $image = $data['row']->warrenty_section_icon;
            }
            /* warrenty_section icon */
            $postData = array(
                    'product_category'          => $this->request->getPost('product_category'),
    ...
    public function image_list($id) 
    {
        ...
        $page_name                  = 'warrenty_section/image-list';        
    ...
    public function image_add($id)
    {
        ...
        $page_name                  = 'warrenty_section/add-edit-image';
        $data['row']                = [];
        $data['project_id']           = $id;
        $data['project']              = $this->data['model']->find_data($this->data['table_name'], 'row', ['id' => $id]);       
