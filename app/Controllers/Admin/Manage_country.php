...
use DB;
class Manage_country extends BaseController {
	...
	public function __construct()
    {
        ...
        $this->data = array(
            ...
            'session'       => $session,
            'module'        => 'Country',
            'controller'    => 'manage_country',
            'table_name'    => 'sms_countries',
            'primary_key'   => 'id'
        );
    }
    ...
    public function add()
    {
        ...
        if($this->request->getMethod() == 'post') {            
            /* image upload */
            $file = $this->request->getFile('flag');
            $originalName = $file->getClientName();
            $fieldName = 'flag';
            if($originalName!='') {
                $upload_array = $this->common_model->upload_single_file($fieldName,$originalName,'country','image');
                if($upload_array['status']) {
                    $flag = $upload_array['newFilename'];
                } else {
    ...
    public function edit($id)
    {
        ...
        if($this->request->getMethod() == 'post') {            
            ...
            if($originalName!='') {
                ...
                if($data['row']->flag!='') {
                    unlink('uploads/country/'.$data['row']->flag);  
                }                
