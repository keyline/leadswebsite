...
class Manage_project_process extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->data = array(
            'model' => $model,
            'session' => $session,
            'module' => 'Project_process',
            'controller' => 'manage_project_process',
            'table_name' => 'project_process',
            'primary_key' => 'id'
        );
    }
    public function index()
    {
        ...
        $page_name = 'project_process/image-list';
    ...
    public function add()
    {
        ...
        $page_name = 'project_process/add-edit-image';
        $data['row'] = [];
        $data['projectprocess'] = $this->data['model']->find_data('project_process', 'array', ['published!=' => 3]);
        if ($this->request->getMethod() == 'post') {
            ...
            $fieldName = 'pdf';
            if ($originalName != '') {
                $upload_array = $this->common_model->upload_single_file($fieldName, $originalName, 'projects_process', 'pdf');
                if ($upload_array['status']) {
                    $pdf = $upload_array['newFilename'];
                } else {
